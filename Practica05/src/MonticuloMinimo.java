...
public class MonticuloMinimo<T extends ComparableIndexable<T>> implements Collection<T>{
    ...
    */
    ...
    //Metodo auxiliar 
    public void heap(int arr[], int i, int size){
        int smallest = i; //inicializamos el indice del menor
        int left = 2 * i + 1;
        int right = 2 * i + 2;
 
        //si el hijo izquierdo es menor que el padre
        if (left < size && arr[left] < arr[i])
            smallest = left;
 
        // Si el hijo derecho es menor que el padre
        if (right < size && arr[right] < arr[smallest])
            smallest = right;

        // Si el menor es diferente al padre
        if (smallest != i) {
            swap2(arr, i, smallest);
            heap(arr, smallest, size);
        }
    }
    ...
    //Metdo que nos dice si un arreglo en el parametro es un monticulo minimo 
    public static boolean esMontMin(int[] A){
        // base case
        if (A.length <= 1) {
            return true;
        }
    ...
    // Funcion para convertir un Max heap a un Min heap en O(n)
       public void MontMax_MontMin(int[] A){
          int i = (A.length - 2) / 2;
           while (i >= 0) {
            heap(A, i--, A.length);
           }
